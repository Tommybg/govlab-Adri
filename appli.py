...
6. Maintain a professional and constructive tone throughout the interaction.s
...
class ExcelLoader(BaseLoader):
    """Load Excel files into documents using in-memory processing."""
    
    def __init__(self, file_content, filename):
        """Initialize with file content and filename."""
        self.file_content = file_content
        self.filename = filename
    
    def load(self) -> list[Document]:
        """Load and convert Excel content into documents."""
        try:
            # Create BytesIO object from file content
            excel_buffer = io.BytesIO(self.file_content)
            
            # Read all sheets from the Excel file
            df_dict = pd.read_excel(excel_buffer, sheet_name=None, engine='openpyxl')
            documents = []
            
            # Process each sheet
            for sheet_name, df in df_dict.items():
                # Convert sheet to string, maintaining column names and structure
        ...
        for uploaded_file in uploaded_files:
            try:
                ...
                if file_extension in ['.xlsx', '.xls']:
                    # Use in-memory Excel processing
                    loader = ExcelLoader(bytes_data, uploaded_file.name)
                
                elif file_extension in ['.pdf', '.PDF']:
                    # Create temporary buffer for PDF
                    pdf_buffer = io.BytesIO(bytes_data)
                    # For PDFs, we need to pass the buffer differently
                    class BufferLoader(PyPDFLoader):
                        ...
                        def load(self):
                            import PyPDF2
                            pdf = PyPDF2.PdfReader(self.file_obj)
                            return [
                                Document(
                                    page_content=page.extract_text(),
                                    metadata={"source": self.filename, "page": i}
                                )
                                for i, page in enumerate(pdf.pages)
                            ]
                    
                    loader = BufferLoader(pdf_buffer, uploaded_file.name)
                    ...
                    class CSVBufferLoader(CSVLoader):
                        def __init__(self, file_obj, filename):
                            self.file_obj = file_obj
                            self.filename = filename
                        ...
                        def load(self):
                            ...
                            for i, row in enumerate(reader):
                                content = "\n".join(f"{headers[i]}: {value}" for i, value in enumerate(row))
                                metadata = {"source": self.filename, "row": i}
                                docs.append(Document(page_content=content, metadata=metadata))
                            return docs
                            
                    loader = CSVBufferLoader(csv_buffer, uploaded_file.name)
                
                elif file_extension == '.docx':
                    # Process DOCX in memory
                    docx_buffer = io.BytesIO(bytes_data)
                    
                    class DocxBufferLoader:
                        def __init__(self, file_obj, filename):
                            self.file_obj = file_obj
                            self.filename = filename
                        ...
                        def load(self):
                            import docx2txt
                            text = docx2txt.process(self.file_obj)
                            metadata = {"source": self.filename}
                            return [Document(page_content=text, metadata=metadata)]
                    
                    loader = DocxBufferLoader(docx_buffer, uploaded_file.name)
                    ...
                    continue  # Skip the loader.load() call
